...
#define  BSP_COM1_SEN               DEF_ENABLED
#define  BSP_COM2_SEN               DEF_ENABLED
#define  BSP_COM3_SEN               DEF_ENABLED
#define  BSP_COM4_SEN               DEF_DISABLED

/***********************************************
* 描述： 
*/
#if (BSP_COM1_REN == DEF_ENABLED)
#define MBREN1_GPIO_PIN        GPIO_Pin_15//8             /* PA.08 */
...
#define MBREN4_GPIO_PIN        GPIO_Pin_0//15             /* PE.15 */
#define MBREN4_GPIO_PORT       GPIOD//A
#define MBREN4_GPIO_RCC        RCC_APB2Periph_GPIOD//A
#endif
/*
*********************************************************************************************************
*                                             LOCAL VARIABLES
...
void  USARTx_RxTxISRHandler (MODBUS_CH *pch)
{
    ...
    * 描述：接收中断
    */
    if (SET == USART_GetFlagStatus(pch->USARTx, USART_FLAG_RXNE)) {
        rx_data = (uint16_t)(pch->USARTx->DR & (uint16_t)0x01FF);
        pch->RxCtr++;
        MB_RxByte(pch, rx_data);                                        /* Pass character to Modbus to process                  */
        USART_ClearITPendingBit(pch->USARTx, USART_IT_RXNE);            /* Clear the USART2 receive interrupt.                  */
    }
    /***********************************************
    * 描述：发送中断
    */
    if (SET == USART_GetFlagStatus(pch->USARTx, USART_FLAG_TC)) {
        USART_ClearITPendingBit(pch->USARTx, USART_IT_TC);              /* Clear the USART2 receive interrupt.                  */
        pch->TxCtr++;
...
void  MB_CommTxIntDis (MODBUS_CH  *pch)
{
    ...
    USART_ITConfig(pch->USARTx, USART_IT_TXE, DISABLE);
    CPU_CRITICAL_EXIT();
...
void  MB_CommTxIntEn (MODBUS_CH  *pch)
{
    ...
    USART_ITConfig(pch->USARTx, USART_IT_TXE, ENABLE);
    CPU_CRITICAL_EXIT();
