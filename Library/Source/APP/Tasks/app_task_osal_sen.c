...
碳原子数 10-22
*/
#define OIL_DENSITY_MIN             0.7800
#define OIL_DENSITY_MAX             0.8900
...
#if CHANNEL_SEL == 1//AUTO_SEL
            static INT16U sample_ctr   = 0;
            
            if ( ++sample_ctr > 20) {
                if ( abs(630 + (int)Ctrl.Sen.C.DiffPressure ) < 50 ) {
                    chang_flag  = TRUE;
                    sample_ctr  = 20;
                } else if ( abs(760 + (int)Ctrl.Sen.C.DiffPressure ) < 50 ) {
                    chang_flag  = TRUE;
                    sample_ctr  = 20;
                } else if ( sample_ctr > 40 ){
                    sample_ctr  = 0;
                }
            }
...
#endif
            if ( chang_flag )
                AdcValue    = (double)BSP_ADCGetValue( 3 );
            else
                AdcValue    = (double)BSP_ADCGetValue( 2 );
    ...
    */
    if( task_event & OS_EVT_SEN_TICKS ) {
        ...
        */
        ...
        Ctrl.Sen.C.CalcDensity              = Ctrl.Sen.Para.dat.Density_B + (float)( Ctrl.Sen.Para.dat.Density_K * 10.000 * (double)(Ctrl.Sen.C.DiffPressure)      / (double)((double)Ctrl.Sen.Para.dat.ChkedDiffHight * Ctrl.Sen.C.Gravity));
        Ctrl.Sen.C.CalcDensitySec           = Ctrl.Sen.Para.dat.Density_B + (float)( Ctrl.Sen.Para.dat.Density_K * 10.000 * (double)(Ctrl.Sen.C.DiffPressureSec)   / (double)((double)Ctrl.Sen.Para.dat.ChkedDiffHight * Ctrl.Sen.C.Gravity));
        Ctrl.Sen.C.CalcDensityMin           = Ctrl.Sen.Para.dat.Density_B + (float)( Ctrl.Sen.Para.dat.Density_K * 10.000 * (double)(Ctrl.Sen.C.DiffPressureMin)   / (double)((double)Ctrl.Sen.Para.dat.ChkedDiffHight * Ctrl.Sen.C.Gravity));
        Ctrl.Sen.C.CalcDensityAll           = Ctrl.Sen.Para.dat.Density_B + (float)( Ctrl.Sen.Para.dat.Density_K * 10.000 * (double)(Ctrl.Sen.C.DiffPressureAll)   / (double)((double)Ctrl.Sen.Para.dat.ChkedDiffHight * Ctrl.Sen.C.Gravity));            
        Ctrl.Sen.C.CalcDensitySample        = Ctrl.Sen.Para.dat.Density_B + (float)( Ctrl.Sen.Para.dat.Density_K * 10.000 * (double)(Ctrl.Sen.C.DiffPressureSample)/ (double)((double)Ctrl.Sen.Para.dat.ChkedDiffHight * Ctrl.Sen.C.Gravity));            
        ...
        */
        case 3:
            ...
            */
                ...
                */
                //} else {
                CPU_FP32    dHight   = abs( (int)(Ctrl.Sen.C.CalcDiffHight - Ctrl.Sen.Para.dat.ChkedDiffHight));
                if ( ( dHight > 900  ) || ( dHight < 700 ) ) {
                    SenSel  = 0;
                } else {
                    SenSel  = 2;
                }
...
#define DIFF_P  250
    ...
    */
    if( task_event & OS_EVT_SEN_ERR_CHK ) {
        ...
        * 描述： 如果是双传感器模块
        */
        if ( Ctrl.Para.dat.Sel.udat.DevSel == 0 ) {
            if ( pSen->CurrentSec < 3.0 ) {
                /***********************************************
                * 描述： 高位传感器为零时不检测断开，只检测在3mA以下和0.1以上
                */ 
    ...
    * 描述： 更新参数
    */
    if( task_event & OS_EVT_SEN_UPDATE_PARA ) {
        APP_ParaInit();
        return ( task_event ^ OS_EVT_SEN_UPDATE_PARA );
    }
    return 0;
}
...
void APP_ParaInit(void)
{
    ...
    */
    ...
    // 获取偏移值
    Ctrl.Sen.C.OffsetHight          = (INT16S)App_fParaFilter(Ctrl.Para.dat.Vtab.Tab.Offset,0,0,1000);
    if ( fabs(Ctrl.Para.dat.Vtab.Tab.Offset) < 50.0 )
        Ctrl.Sen.C.OffsetHight     += (INT16S)Ctrl.Para.dat.Vtab.Tab.Offset;
    ...
    Ctrl.Sen.Para.dat.ChkedDiffHight= (float)App_fParaFilter(Ctrl.Sen.Para.dat.ChkedDiffHight,
                                                             ...
                                                             DIFF_HIGHT_MAX);
    // 获取密度补偿参数
    Ctrl.Sen.Para.dat.Density_K     = (float)App_fParaFilter(Ctrl.Sen.Para.dat.Density_K,
                                                             1,
                                                             0.5,
                                                             1.5);
    // 获取斜率补偿参数
    Ctrl.Sen.Para.dat.Density_B     = (float)App_fParaFilter(Ctrl.Sen.Para.dat.Density_B,
                                                             0,
                                                             -0.5,
                                                             0.5);
    ...
    * 描述： 如果是显示模块
    */
    Ctrl.Para.dat.Sel.udat.DevSel = 2;              // 初始化为显示模块
#endif
    /***********************************************
    * 描述： 电路板校准电初始化
    */
