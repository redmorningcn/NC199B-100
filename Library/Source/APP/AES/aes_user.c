...
static INT08U      mbszIvec[16] = {0X12, 0x34, 0x56, 0x78,0X87, 0x65, 0x56, 0x78,
                                 0x87, 0x65, 0x43, 0x21,0x12, 0x34, 0x43, 0x21};
static AES_KEY     mbaesKey;
static INT08U      mbuserIvec[16];
#endif

static AES_KEY     aesKey;                            		            // AES 扩展密钥
static INT08U 	    userKey[AES_BYTES+1];//    = "snlion>2014+2^99" / "2014<snlion>2^99";		// 解密密钥 16字节
static INT08U 	    userIvec[AES_BYTES+1];//   = "4307783638394734";		// 解密向量 16字节
...
void AES_CbcInitKeyForApp(BOOL dec)
{
	...
	* 描述： 设置密钥
	*/
    AES_GetUserKeyForApp((INT08U *)userKey);
...
void AES_CbcDecrypt(const unsigned char *pIn, unsigned char *pOut, long len, unsigned char dec)
{
	...
	* 描述： 
	*/
    AES_CbcEnOrDecrypt((INT08U*)pIn,                // 待解密缓冲区
                   (INT08U*)pOut,                   // 解密后的缓冲区
                    len,				            // 缓冲区长度
                   (AES_KEY*)&aesKey, 		        // 扩展密钥
                   (INT08U*)userIvec, 		        // 向量
 ...
 * 修改日期：
 *******************************************************************************/
INT16U MB_AESCbcDecrypt(INT08U *buf, INT16U len)
{
#if MB_AES_EN == DEF_ENABLED    
    ...
    * 描述： 设置密钥向量
    */
    memcpy((INT08U*)mbuserIvec,(INT08U*)&mbszIvec[0], 16);// 向量
    /***********************************************
    ...
    */
    ...
    AES_CbcEnOrDecrypt((INT08U*)buf,                 // 待解密缓冲区
                       (INT08U*)buf,                 // 解密后的缓冲区
                        len,				        // 缓冲区长度
                       (AES_KEY*)&mbaesKey, 		// 扩展密钥
                       (INT08U*)mbuserIvec, 		    // 向量
                        AES_DECRYPT);		        // 加密
    /***********************************************
    ...
    */
      ...
      INT08U i   = buf[len-1];                       // 获取填充值
      INT08U *p  = &buf[len-i];                      // 指向开始填充的字节地址
      for (i-=1;i > 0;i--,p++) {
 ...
 * 修改日期：
 *******************************************************************************/
INT16U MB_AESCbcEncrypt(INT08U *buf, INT16U len)
{
#if MB_AES_EN == DEF_ENABLED
    INT08U       temp;
    ...
    */
    ...
    if ( temp != 0) {
      temp    = 16 - temp;
      memset((INT08U*)&buf[len],temp,temp);
      len = len + temp;
    }
    AES_CbcEnOrDecrypt((INT08U*)&buf[0],             // 待解密缓冲区
                       (INT08U*)&buf[0],             // 解密后的缓冲区
                        len,	                    // 缓冲区长度
                       (AES_KEY*)&mbaesKey, 		// 扩展密钥
                       (INT08U*)mbuserIvec, 		    // 向量
                        AES_ENCRYPT);		        // 加密
