...
//4.8.10 校准顺序
//偏移量校准是否应在增益校准之前，或应反过
//来？本节将推荐合理的校准顺序。
//1． 若希望测量电压/电流和功率/能量信号中的直
//流成分，则应先进行直流偏移量校准。 若希望
//去除电压和电流信号中的直流成分（通过选择
//状态寄存器中的HPF位）， 则不需进行DC偏移
//量寄存器。 一旦选择了电压或电流的HPF，则
//功率/能量信号中的直流成分将从功率/能量结
//果中去除。
//2． 若希望将电能寄存器的准确度保持在± 0.1%
//（相对于电压/电流输入的参考校准电平）内，
//则应接下来进行交流或直流增益校准。
//3． 最后进行电压和电流通道的交流偏移量校准。
//只需将两个通道的“＋”“－”输入端接地，
//然后执行交流偏移量校准程序。
//需注意的是如果某一通道进行了直流偏移量
//校准，然后又进行了增益校准，则该通道DC偏移
//量寄存器的值需乘以一个系数， 它等于该通道的新
//的增益寄存器的值。 例如电压通道直流偏移量校准
//后电压通道DC偏移量寄存器的值位0x0001AC＝
//0.0000510（ d）， 假设电压通道增益寄存器的值在
//执行直流偏移量校准过程中为缺省值1.000 … ，则
//交流和直流增益校准后， 电压通道增益寄存器的值
//变为0x4020A3＝ 1.0019920（ d）， 则电压通道DC
//偏移量寄存器的值为0x0001AD=0.0000511(d)，等
//于1.0019920× 0.0000510。
